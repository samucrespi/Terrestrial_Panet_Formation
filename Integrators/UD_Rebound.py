...
#  'SPH.table'). The collision outcome is either 0, 1 or 2 bodies plus
#  a certain amount of unresolved mass (UD) that is localized into
#  circular bins (see Leinhardt & Richardson 2005 (LR05)for a more
#  detailed description).
#  The effects of the UD on the resolved bodies are mass accretion
#  (eq 3-4 LR05) and angular momentum conservation (eq. 5 LR05). As in
#  LR05, no drag force is included.
#
# It is based on FwoD_Rebound.py v4
#
...
def collision_solver(sim_pointer, collision):
	...
	if Nbig>0:
		...
		#vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv	
			...
			print('   wf: {:.4f} [%]'.format(survivors[i][3]))
	...
	# fill the circular bins
	if mfr>0:
		ibin=int(np.floor((rcol-amin)/da))
		oldm,oldwf=Cbins[ibin]
		Cbins[ibin]=[oldm+mfr,(oldwf*oldm+wffr*mfr)/(oldm+mfr)]
...
#---------------------------
# Unresolved Debris effect
#---------------------------

def UD_effect(delta_t,M=1.):
	Cbin_loss=np.zeros(NCbins)
	sim.move_to_hel()
	
	# evolve particles
	for p in ps[NGG+1:]:
		pa=p.a
		if pa>amax or pa<amin: continue
		ibin=int(np.floor((pa-amin)/da))
		md,wfd=Cbins[ibin]
		if md>0:
			abin=amin+da*(0.5+ibin)
			dm=np.pi*p.r*p.r*p.e*np.sqrt(M/pa)*delta_t*md/(2.*Dz*abin*da)
			pm0=p.m*1.
			# mass accretion
			p.m=pm0+dm
			dm=p.m-pm0	# to avoid rounding errors
			# Cbin mass depletion
			Cbin_loss[ibin]=Cbin_loss[ibin]-dm
			# water accretion
			p.params['wf']=(p.params['wf']*pm0+Cbins[ibin][1]*dm)/p.m
			# momentum conservation
			thd=np.arctan2(p.y,p.x)
			vkd=np.asarray([-np.sin(thd),np.cos(thd)])*np.sqrt(M/pa)
			mR=pm0/p.m
			p.vx=vkd[0]+mR*(p.vx-vkd[0])
			p.vy=vkd[1]+mR*(p.vy-vkd[1])
			p.vz=mR*p.vz
	
	# update Cbins
	for i in range(NCbins):
		Cbins[i][0]=Cbins[i][0]+Cbin_loss[i]
		if Cbins[i][0]<0: Cbins[i][0]=[0.,0.]
...
def initialize_Cbins_file(t):
	f=open(Cb_file,'w+')
	f.write('# time [yr]\t[mfr[MSUN],wffr[%]]xNCbins\n')
	f.close()
	save_Cbins(t,Cbins)
...
def save_Cbins(t,Cbins):
	f=open(Cb_file,'a')
	f.write('{}\t'.format(t))
	for b in Cbins: f.write('{}\t{}\t'.format(b[0],b[1]))
	f.write('\n')
	f.close()		
...
# Cbins file
Cb_file='{}/Cbins.dat'.format(scenario)
...
#circular bins
da,amin,amax=0.1,0.25,4.45	# bin size [AU], bin boudaries [AU]
NCbins=int((amax-amin)/da)
Cbins=[[0.,0.] for i in range(NCbins)]
Cb_freq=100				# apply the UD effect every Cb_freq time steps
Dz=1.					# thickness of the bin [AU]
...
# --- Unresolved Debris
if glob.glob(Cb_file)==[]: initialize_Cbins_file(tmin/np.pi/2.)
else:
	f=np.loadtxt(Cb_file)[-1][1:]
	Cbins=[[f[i*2],f[i*2+1]] for i in range(int(len(f)/2))]
...
for t in times:
	...
	# removing-ps stepping
	tnext_rm=sim.t+rem_freq*sim.dt
	while tnext_rm<t:
	
		# Cbins stepping
		tnext_cb=sim.t+Cb_freq*sim.dt
		while tnext_cb<tnext_rm:
			sim.integrate(tnext_cb)
			UD_effect(Cb_freq*sim.dt)		# applies the UD effect
			tnext_cb+=Cb_freq*sim.dt
			
		sim.integrate(tnext_rm)
		remove_ps() 	# remove particles
		UD_effect(Cb_freq*sim.dt-(tnext_cb-tnext_rm))		# applies the UD effect
		tnext_rm+=rem_freq*sim.dt
	...
	sim.integrate(t)
	remove_ps() 	# remove particles
	UD_effect(Cb_freq*sim.dt-(tnext_cb-t))		# applies the UD effect

	if save_checkpoints:
		save_data(sim.t/np.pi/2.,ps,scenario)
		save_Cbins(sim.t/np.pi/2.,Cbins)
