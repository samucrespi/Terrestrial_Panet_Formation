...
#  'SPH.table') and generates fragments as in Clement et al. 2019b (see
#  Poon et al. 2020 for a more detailed description).
#
# It is based on FwoD_Rebound.py v4
# 
# Clement et al. 2019b linked here:
#	https://ui.adsabs.harvard.edu/abs/2019Icar..321..778C/abstract
# Poon et al. 2020 linked here:
#	https://ui.adsabs.harvard.edu/abs/2020MNRAS.491.5595P/abstract

# NB: all the units are in "rebound" units (G=1) except where otherwise
#     specified
#
# Version note:
#  - v1.1: bug fixed - evaluate chi and psi even for Nbig=0
#  - v1.2: improved remove_ps()
#  - v1.3: bug fixed - no fragments were produced if Nbig=0
#
#----------------------------------------------------------------------
#----------------------------------------------------------------------
...
def collision_solver(sim_pointer, collision):
	...
	# move survivors to their center of mass SoC
	if Nbig==1:
		survivors[0][0][:]=np.zeros(3)
		survivors[0][1][:]=np.zeros(3)
	if Nbig==2:
		xsCoM=(survivors[0][0][:]*survivors[0][2]+survivors[1][0][:]*survivors[1][2])/(survivors[0][2]+survivors[1][2])
		vsCoM=(survivors[0][1][:]*survivors[0][2]+survivors[1][1][:]*survivors[1][2])/(survivors[0][2]+survivors[1][2])
		survivors[0][0][:]=survivors[0][0][:]-xsCoM
		survivors[0][1][:]=survivors[0][1][:]-vsCoM
		survivors[1][0][:]=survivors[1][0][:]-xsCoM
		survivors[1][1][:]=survivors[1][1][:]-vsCoM
	...
	if Nfr>0:
		...
		for i in range(Nfr):
			...
			debris.append([versor*HR,versor*vd,md,wffr])
		...
		if debris_plot:
			# Nbig=1 case
			if Nbig==1:
				Rfr=-survivors[0][0]*survivors[0][2]/mfr
				Vfr=-survivors[0][1]*survivors[0][2]/mfr
			
				import matplotlib.pyplot as plt
				DT=1.e-2
				fig, axs = plt.subplots(2,figsize=(6,7),gridspec_kw={'height_ratios': [3,1]})
			
				corr=1e2/REAR	# plotted radius is 10x the real one
				s1,sfr=get_radius(survivors[0][2],0)*corr,get_radius(mfr,0)*corr
				...
				axs[1].set_ylim(-1.e-2/2/3,1.e-2/2/3)
	...
	for i in range(Nfr):
		...
		# update parameters
		sim.add(m=debris[i][2],r=drad,x=debris[i][0][0],y=debris[i][0][1],z=debris[i][0][2],vx=debris[i][1][0],vy=debris[i][1][1],vz=debris[i][1][2])
		sim.particles[i+Nps].params['wf']=debris[i][3]

	#vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
	print('\n Debris:')
	print('   Nfr: {}'.format(Nfr))
	print('   tot_mfr: {:.4f} [MEAR]'.format(mfr/MEAR))
	print('   wffr: {:.2e} %\n'.format(wffr))
	#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^	
...
def save_col_out(t,Nbig,Nfr,mfr,wffr):
	f=open(sp_file,'a+')
	f.write('  ~~~ Collision at t={} yr:   Nbig={}   Nfr={}   mfr={:.4f} MEAR   wffr={:.2e} %\n'.format(t/np.pi/2.,Nbig,Nfr,mfr/MEAR,wffr))
	f.close()	
